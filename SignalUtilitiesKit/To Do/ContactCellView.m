...
- (void)configureWithRecipientId:(NSString *)recipientId
{
    ...
    [self.primaryStorage.dbReadConnection readWithBlock:^(YapDatabaseReadTransaction *transaction) {
        self.thread = [TSContactThread getThreadWithContactBChatID:recipientId transaction:transaction];
    }];
    ...
    if (isNoteToSelf) {
        ...
        self.nameLabel.text = [[LKStorage.shared getContactWithBChatID:recipientId] displayNameFor:context] ?: recipientId;
    }
...
- (void)configureWithThread:(TSThread *)thread
{
    ...
    BOOL isNoteToSelf
        = ([thread isKindOfClass:TSContactThread.class] && [((TSContactThread *)thread).contactBChatID isEqualToString:self.tsAccountManager.localNumber]);
    if (isNoteToSelf) {
        threadName = NSLocalizedString(@"NOTE_TO_SELF", @"Label for 1:1 conversation with yourself.");
    }

    if ([thread isKindOfClass:[TSContactThread class]]) {
        self.recipientId = ((TSContactThread *)thread).contactBChatID;
...
- (void)updateProfileName
{
    ...
    SNContactContext context = [SNContact contextForThread:self.thread];
    NSString *displayName = [[LKStorage.shared getContactWithBChatID:publicKey] displayNameFor:context] ?: publicKey;
