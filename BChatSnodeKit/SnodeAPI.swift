...
public final class SnodeAPI : NSObject {
        ...
        Threading.workQueue.async {
            attempt(maxRetryCount: 4, recoveringOn: Threading.workQueue) {
                HTTP.execute(.post, url, parameters: parameters, useSeedNodeURLSession: true).map2 { json -> Set<Snode> in
                    guard let intermediate = json["result"] as? JSON, let rawSnodes = intermediate["master_node_states"] as? [JSON] else { throw Error.snodePoolUpdatingFailed }
                ...
                SNLog("Got mnode pool from seed node: \(target).")
    ...
    private static func getSnodePoolFromSnode() -> Promise<Set<Snode>> {
        ...
        let snodePoolPromises: [Promise<Set<Snode>>] = snodes.map { snode in
            return attempt(maxRetryCount: 4, recoveringOn: Threading.workQueue) {
                ...
                return invoke(.beldexDaemonRPCCall, on: snode, parameters: parameters).map2 { rawResponse in
                    guard let json = rawResponse as? JSON, let intermediate = json["result"] as? JSON,
                        let rawSnodes = intermediate["master_node_states"] as? [JSON] else {
                        throw Error.snodePoolUpdatingFailed
                    }
                    return Set(rawSnodes.compactMap { rawSnode in
                        guard let address = rawSnode["public_ip"] as? String, let port = rawSnode["storage_port"] as? Int,
                            let ed25519PublicKey = rawSnode["pubkey_ed25519"] as? String, let x25519PublicKey = rawSnode["pubkey_x25519"] as? String, address != "0.0.0.0" else {
                            SNLog("Failed to parse mnode from: \(rawSnode).")
                            return nil
                        }
                        return Snode(address: "https://\(address)", port: UInt16(port), publicKeySet: Snode.KeySet(ed25519Key: ed25519PublicKey, x25519Key: x25519PublicKey))
                    })
    ...
    public static func getBChatID(for onsName: String) -> Promise<String> {
        ...
        let promises = (0..<validationCount).map { _ in
            return getRandomSnode().then2 { snode in
                attempt(maxRetryCount: 4, recoveringOn: Threading.workQueue) {
                    invoke(.beldexDaemonRPCCall, on: snode, parameters: parameters)
                }
            }
        }
        ...
        when(resolved: promises).done2 { results in
            ...
            for result in results {
                ...
                case .fulfilled(let rawResponse):
                    ...
                    if isArgon2Based {
                        ...
                        guard let bchatIDAsData = sodium.secretBox.open(authenticatedCipherText: ciphertext, secretKey: key, nonce: nonce) else {
                            return seal.reject(Error.decryptionFailed)
                        }
                        bchatIDs.append(bchatIDAsData.toHexString())
                    } else {
                        ...
                        guard ciphertext.count >= (bchatIDByteCount + sodium.aead.xchacha20poly1305ietf.ABytes) else { // Should always be equal in practice
                            return seal.reject(Error.decryptionFailed)
                        }
                        guard let bchatIDAsData = sodium.aead.xchacha20poly1305ietf.decrypt(authenticatedCipherText: ciphertext, secretKey: key, nonce: nonce) else {
                            return seal.reject(Error.decryptionFailed)
                        }
                        bchatIDs.append(bchatIDAsData.toHexString())
                    }
            ...
            guard bchatIDs.count == validationCount && Set(bchatIDs).count == 1 else { return seal.reject(Error.validationFailed) }
            seal.fulfill(bchatIDs.first!)
        }
        return promise
    }
    ...
    private static func parseSnodes(from rawResponse: Any) -> Set<Snode> {
        guard let json = rawResponse as? JSON, let rawSnodes = json["mnodes"] as? [JSON] else {
    ...
    internal static func handleError(withStatusCode statusCode: UInt, json: JSON?, forSnode snode: Snode, associatedWith publicKey: String? = nil) -> Error? {
        ...
        func handleBadSnode() {
            ...
            SNLog("Couldn't reach mnode at: \(snode); setting failure count to \(newFailureCount).")
        ...
        case 406:
            SNLog("The user's clock is out of sync with the master node network.")
