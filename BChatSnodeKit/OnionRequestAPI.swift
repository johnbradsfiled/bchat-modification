...
/// See the "Onion Requests" section of [The BChat Whitepaper](https://arxiv.org/pdf/2002.04609.pdf) for more information.
public enum OnionRequestAPI {
    ...
    public enum Destination : CustomStringConvertible {
        ...
        public var description: String {
            switch self {
            case .snode(let snode): return "Master node \(snode.ip):\(snode.port)"
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            HTTP.execute(.get, url, timeout: timeout).done2 { json in
                ...
                if version >= "2.0.7" {
                    ...
                    SNLog("Unsupported mnode version: \(version).")
    ...
    private static func getGuardSnodes(reusing reusableGuardSnodes: [Snode]) -> Promise<Set<Snode>> {
        if guardSnodes.count >= targetGuardSnodeCount {
            ...
            SNLog("Populating guard mnode cache.")
            var unusedSnodes = SnodeAPI.snodePool.subtracting(reusableGuardSnodes) // Sync on BeldexAPI.workQueue
            ...
            func getGuardSnode() -> Promise<Snode> {
                ...
                SNLog("Testing guard mnode: \(candidate).")
    ...
    public static func sendOnionRequest(_ request: NSURLRequest, to server: String, target: String = "/beldex/v3/lsrpc", using x25519PublicKey: String) -> Promise<JSON> {
    ...
    public static func sendOnionRequest(with payload: JSON, to destination: Destination) -> Promise<JSON> {
        ...
        promise.catch2 { error in // Must be invoked on Threading.workQueue
            ...
            } else if let message = json?["result"] as? String, message == "Beldex Server error" {
                // Do nothing
