...
final class QRCodeVC : BaseVC, UIPageViewControllerDataSource, UIPageViewControllerDelegate, OWSQRScannerDelegate {
    ...
    fileprivate func startNewPrivateChatIfPossible(with hexEncodedPublicKey: String) {
        if !ECKeyPair.isValidHexEncodedPublicKey(candidate: hexEncodedPublicKey) {
            let alert = UIAlertController(title: NSLocalizedString("invalid_bchat_id", comment: ""), message: NSLocalizedString("Please check the BChat ID and try again.", comment: ""), preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: NSLocalizedString("BUTTON_OK", comment: ""), style: .default, handler: nil))
            presentAlert(alert)
        } else {
            let thread = TSContactThread.getOrCreateThread(contactBChatID: hexEncodedPublicKey)
...
private final class ViewMyQRCodeVC : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        titleLabel.font = Fonts.boldOpenSans(ofSize: isIPhone5OrSmaller ? CGFloat(40) : Values.massiveFontSize)
        ...
        explanationLabel.font = Fonts.OpenSans(ofSize: Values.mediumFontSize)
