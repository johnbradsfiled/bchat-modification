...
enum MockDataGenerator {
    ...
    static func generateMockData() {
        ...
        Storage.shared.write { anyTransaction in
            ...
            _ = TSContactThread.getOrCreateThread(withContactBChatID: "MockDatabaseThread", transaction: transaction)
            ...
            (0..<dmThreadCount).forEach { threadIndex in
                ...
                let randomBChatId: String = KeyPairUtilities.generate(from: data).x25519KeyPair.hexEncodedPublicKey
                ...
                // Generate the thread
                let thread: TSContactThread = TSContactThread.getOrCreateThread(withContactBChatID: randomBChatId, transaction: transaction)
                thread.shouldBeVisible = true
                
                // Generate the contact
                let contact = Contact(bchatID: randomBChatId)
                ...
                (0..<numMessages).forEach { index in
                    ...
                    message.sender = (isIncoming ? randomBChatId : userSessionId)
            ...
            (0..<closedGroupThreadCount).forEach { threadIndex in
                ...
                (0..<numGroupMembers).forEach { _ in
                    ...
                    let randomBChatId: String = KeyPairUtilities.generate(from: contactData).x25519KeyPair.hexEncodedPublicKey
