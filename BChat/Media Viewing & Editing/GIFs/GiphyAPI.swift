...
extension GiphyError: LocalizedError {
    public var errorDescription: String? {
        ...
        case .fetchFailure:
            return NSLocalizedString("GIF_PICKER_ERROR_FETCH_FAILURE", comment: "Error displayed when there is a failure fetching a GIF from the remote master.")
        }
    }
}
...
@objc class GiphyAPI: NSObject {
    ...
    public func trending() -> Promise<[GiphyImageInfo]> {
        guard let sessionManager = giphyAPISessionManager() else {
            Logger.error("Couldn't create bchat manager.")
