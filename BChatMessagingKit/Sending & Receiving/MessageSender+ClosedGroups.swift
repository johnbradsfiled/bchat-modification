...
extension MessageSender {
    ...
    public static func createClosedGroup(name: String, members: Set<String>, transaction: YapDatabaseReadWriteTransaction) -> Promise<TSGroupThread> {
        ...
        let groupPublicKey = Curve25519.generateKeyPair().hexEncodedPublicKey // Includes the "bd" prefix
        ...
        for member in members {
            let thread = TSContactThread.getOrCreateThread(withContactBChatID: member, transaction: transaction)
            thread.save(with: transaction)
    ...
    public static func sendLatestEncryptionKeyPair(to publicKey: String, for groupPublicKey: String, using transaction: YapDatabaseReadWriteTransaction) {
        ...
        let contactThread = TSContactThread.getOrCreateThread(withContactBChatID: publicKey, transaction: transaction)
