...
extension MessageReceiver {
    ...
    public static func handle(_ message: Message, associatedWithProto proto: SNProtoContent, openGroupID: String?, isBackgroundPoll: Bool, using transaction: Any) throws {
        ...
        if let sharedUserDefaults = UserDefaults(suiteName: "group.com.beldex.bchat") {
    ...
    private static func handleConfigurationMessage(_ message: ConfigurationMessage, using transaction: Any) {
        ...
        if isInitialSync || messageSentTimestamp > lastConfigTimestamp {
            ...
            for contactInfo in message.contacts {
                let bchatId = contactInfo.publicKey!
                let contact = (Storage.shared.getContact(with: bchatId, using: transaction) ?? Contact(bchatID: bchatId))
                ...
                if contactInfo.hasIsBlocked && contactInfo.isBlocked {
                    ...
                    if
                        ...
                        let thread: TSContactThread = TSContactThread.fetch(for: bchatId, using: transaction),
    ...
    public static func handleCallMessage(_ message: CallMessage, using transaction: Any) {
        ...
        case .preOffer:
            ...
            // for this call would be dropped because of no BChat call instance
        ...
        case .answer:
            ...
            guard let currentWebRTCBChat = WebRTCBChat.current, currentWebRTCBChat.uuid == message.uuid! else { return }
    ...
    public static func handleVisibleMessage(_ message: VisibleMessage, associatedWithProto proto: SNProtoContent, openGroupID: String?, isBackgroundPoll: Bool, using transaction: Any) throws -> String {
        ...
        if let profile = message.profile {
            ...
            updateProfileIfNeeded(publicKey: bchatId, name: profile.displayName, profilePictureURL: profile.profilePictureURL,
        ...
        // Beldex address insert into Contact DB
            ...
            if let contact: Contact = Storage.shared.getContact(with: userBchatID) {
              ...
              print("--senderBeldexAddress in receiver side contact.beldexAddress---> \(contact.beldexAddress!))")
              Storage.shared.setContact(contact, using: transaction)
            }
    ...
    private static func updateProfileIfNeeded(publicKey: String, name: String?, profilePictureURL: String?,
        ...
        let contact = Storage.shared.getContact(with: publicKey) ?? Contact(bchatID: publicKey) // New API
        // Name
    ...
    private static func updateContactApprovalStatusIfNeeded(
        ...
        if senderSessionId == userPublicKey {
            ...
            guard let contact: Contact = Storage.shared.getContact(with: thread.contactBChatID(), using: transaction) else { return }
