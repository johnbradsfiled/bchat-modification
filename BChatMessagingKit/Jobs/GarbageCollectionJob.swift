                ...
                if finalTypesToCollect.contains(.oldOpenGroupMessages) && db[.trimOpenGroupMessagesOlderThanSixMonths] {
                    ...
                    try db.execute(literal: """
                        ...
                        WHERE \(Column.rowID) IN (
                            ...
                            JOIN \(BChatThread.self) ON (
                                \(SQL("\(thread[.variant]) = \(BChatThread.Variant.community)")) AND
                ...
                if finalTypesToCollect.contains(.orphanedJobs) {
                    ...
                    try db.execute(literal: """
                        ...
                        WHERE \(Column.rowID) IN (
                            SELECT \(job[.rowId])
                            FROM \(Job.self)
                            LEFT JOIN \(BChatThread.self) ON \(thread[.id]) = \(job[.threadId])
                ...
                if finalTypesToCollect.contains(.orphanedOpenGroups) {
                    ...
                    try db.execute(literal: """
                        ...
                        WHERE \(Column.rowID) IN (
                            ...
                            FROM \(OpenGroup.self)
                            LEFT JOIN \(BChatThread.self) ON \(thread[.id]) = \(openGroup[.threadId])
                            WHERE (
                ...
                if finalTypesToCollect.contains(.orphanedProfiles) {
                    ...
                    try db.execute(literal: """
                        ...
                        WHERE \(Column.rowID) IN (
                            ...
                            FROM \(Profile.self)
                            LEFT JOIN \(BChatThread.self) ON \(thread[.id]) = \(profile[.id])
