...
enum _013_SessionUtilChanges: Migration {
    ...
    static let fetchedTables: [(TableRecord & FetchableRecord).Type] = [
        GroupMember.self, ClosedGroupKeyPair.self, BChatThread.self
    ]
    static let createdOrAlteredTables: [(TableRecord & FetchableRecord).Type] = [
        BChatThread.self, Profile.self, GroupMember.self, ClosedGroupKeyPair.self, ConfigDump.self
    ]
    
    static func migrate(_ db: Database) throws {
        // Add `markedAsUnread` to the thread table
        try db.alter(table: BChatThread.self) { t in
        ...
        try db.create(table: TmpGroupMember.self) { t in
            ...
            t.column(.groupId, .text)
                .notNull()
                .references(BChatThread.self, onDelete: .cascade)   // Delete if Thread deleted
        ...
        try BChatThread
            ...
            .updateAll(
                db,
                BChatThread.Columns.pinnedPriority.set(to: 1)
            )
        ...
        /// - PendingReadReceipt
        let threadIdsToDelete: [String] = try BChatThread
            .filter(
                BChatThread.Columns.shouldBeVisible == false &&
                BChatThread.Columns.id != userPublicKey
            )
            .select(.id)
        ...
        ///
        /// **Note:** Since migrations are run when running tests creating a random BChatThread will result in unexpected thread
        /// counts so don't do this when running tests (this logic is the same as in `MainAppContext.isRunningTests`
        if ProcessInfo.processInfo.environment["XCTestConfigurationFilePath"] == nil {
            if (try BChatThread.exists(db, id: userPublicKey)) == false {
                try BChatThread
