...
public extension ClosedGroup {
    ...
    static func removeKeysAndUnsubscribe(
        ...
        struct ThreadIdVariant: Decodable, FetchableRecord {
            let id: String
            let variant: BChatThread.Variant
        }
        
        let threadVariants: [ThreadIdVariant] = try BChatThread
            .select(.id, .variant)
        ...
        if removeGroupData {
            try BChatThread   // Intentionally use `deleteAll` here as this gets triggered via `deleteOrLeave`
                .filter(ids: threadIds)
                .deleteAll(db)
