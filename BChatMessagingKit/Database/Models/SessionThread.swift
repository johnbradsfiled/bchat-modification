...
public struct BChatThread: Codable, Identifiable, Equatable, FetchableRecord, PersistableRecord, TableRecord, ColumnExpressible {
...
public extension BChatThread {
    /// Fetches or creates a BChatThread with the specified id, variant and visible state
    ///
    /// **Notes:**
    /// - The `variant` will be ignored if an existing thread is found
    /// - This method **will** save the newly created BChatThread to the database
    @discardableResult static func fetchOrCreate(
        ...
        guard let existingThread: BChatThread = try? fetchOne(db, id: id) else {
            return try BChatThread(
                id: id,
                variant: variant,
        ...
        try BChatThread
            ...
            .updateAllAndConfig(
                db,
                BChatThread.Columns.shouldBeVisible.set(to: shouldBeVisible)
            )
        ...
        return (try fetchOne(db, id: id))
            .defaulting(
                to: try BChatThread(id: id, variant: variant, shouldBeVisible: desiredVisibility)
                    .saved(db)
            )
    }
    ...
    static func canSendReadReceipt(
        _ db: Database,
        threadId: String,
        threadVariant maybeThreadVariant: BChatThread.Variant? = nil,
        isBlocked maybeIsBlocked: Bool? = nil,
        isMessageRequest maybeIsMessageRequest: Bool? = nil
    ) throws -> Bool {
        let threadVariant: BChatThread.Variant = try {
            try maybeThreadVariant ??
            BChatThread
                .filter(id: threadId)
                .select(.variant)
                .asRequest(of: BChatThread.Variant.self)
        ...
        let threadIsMessageRequest: Bool = BChatThread
            .filter(id: threadId)
            .filter(
                BChatThread.isMessageRequest(
    ...
    static func refreshPinnedPriorities(_ db: Database, adding threadId: String) throws {
        ...
        let thread: TypedTableAlias<BChatThread> = TypedTableAlias()
        ...
        let pinnedPriorityLiteral: SQL = SQL(stringLiteral: BChatThread.Columns.pinnedPriority.name)
            ...
            case (.contact, .standard), (.contact, .silent):
                ...
                if threadIds.contains(currentUserPublicKey) {
                    _ = try BChatThread
                        ...
                        .updateAllAndConfig(
                            ...
                            BChatThread.Columns.pinnedPriority.set(to: 0),
                            BChatThread.Columns.shouldBeVisible.set(to: false)
                        )
                }
                ...
                _ = try BChatThread
                    ...
                    .updateAllAndConfig(
                        db,
                        calledFromConfig: calledFromConfigHandling,
                        BChatThread.Columns.pinnedPriority.set(to: SessionUtil.hiddenPriority),
                        BChatThread.Columns.shouldBeVisible.set(to: false)
                    )
...
public extension BChatThread {
    static func messageRequestsQuery(userPublicKey: String, includeNonVisible: Bool = false) -> SQLRequest<BChatThread> {
        let thread: TypedTableAlias<BChatThread> = TypedTableAlias()
        ...
        return """
            ...
            WHERE (
                \(BChatThread.isMessageRequest(userPublicKey: userPublicKey, includeNonVisible: includeNonVisible))
            )
        """
    }
    ...
    /// `BChatThread.contact` association or it won't work
    static func isMessageRequest(userPublicKey: String, includeNonVisible: Bool = false) -> SQLExpression {
        ...
        return SQL(
            """
                ...
                \(SQL("\(thread[.variant]) = \(BChatThread.Variant.contact)")) AND
    ...
    func isMessageRequest(_ db: Database, includeNonVisible: Bool = false) -> Bool {
        return BChatThread.isMessageRequest(
            id: id,
            variant: variant,
    ...
    func shouldShowNotification(_ db: Database, for interaction: Interaction, isMessageRequest: Bool) -> Bool {
        ...
        if self.variant == .contact && isMessageRequest {
            ...
            if !hasHiddenMessageRequests {
                let numUnreadMessageRequestThreads: Int = (try? BChatThread
