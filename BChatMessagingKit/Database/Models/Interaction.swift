...
public struct Interaction: Codable, Identifiable, Equatable, FetchableRecord, MutablePersistableRecord, TableRecord, ColumnExpressible {
    ...
    internal static let threadForeignKey = ForeignKey([Columns.threadId], to: [BChatThread.Columns.id])
    internal static let linkPreviewForeignKey = ForeignKey(
        [Columns.linkPreviewUrl],
        to: [LinkPreview.Columns.url]
    )
    public static let thread = belongsTo(BChatThread.self, using: threadForeignKey)
    ...
    public var thread: QueryInterfaceRequest<BChatThread> {
    ...
    public func aroundInsert(_ db: Database, insert: () throws -> InsertionSuccess) throws {
        ...
        guard
            let threadVariant: BChatThread.Variant = try? BChatThread
                .filter(id: threadId)
                .select(.variant)
...
public extension Interaction {
        ...
        return try Interaction
            ...
            .joining(
                required: Interaction.thread
                    .aliased(thread)
                    .joining(optional: BChatThread.contact)
                    .filter(
                        // Ignore muted threads
                        BChatThread.Columns.mutedUntilTimestamp == nil ||
                        BChatThread.Columns.mutedUntilTimestamp < dependencies.dateNow.timeIntervalSince1970
                    )
                    .filter(
                        // Ignore message request threads
                        BChatThread.Columns.variant != BChatThread.Variant.contact ||
                        !BChatThread.isMessageRequest(userPublicKey: userPublicKey)
                    )
            )
            .fetchCount(db)
    }
    ...
    static func markAsRead(
        ...
        interactionId: Int64?,
        threadId: String,
        threadVariant: BChatThread.Variant,
        includingOlder: Bool,
